...
| **`cryogenic-plant`** | This building's own style reference, per the reference plan in the art backlog (now `TODO.md`). Every one of the nine gets a different one, so the set does not come back looking like one machine drawn nine times |
...
the art backlog (now `TODO.md`)'s reference plan names the v2 sheet; that sheet is rejected and
...
The sheets are at `concept/vacuum-furnace/options/`, and
...
`concept/vacuum-furnace/five-options-contact-sheet.png`, which holds all five
at reduced size.

The adopted sheet is `concept/vacuum-furnace/adopted/A-sheet.png`.

...
| `big-mining-drill` | This building's assigned style reference in the art backlog (now `TODO.md`), and the drill it derives from — for finish and for the *scale* of a large drill | second |
| *(no format example)* | See the warning in the art backlog (now `TODO.md`): the Drop Crusher's round had one generation come back as the Ignition Array, the building in its format sheet | — |
...
All five sheets are in `concept/ballast-drill/options/`.
...
Locked 2026-09-10. The sheet is `concept/ballast-drill/adopted/B-sheet.png`;

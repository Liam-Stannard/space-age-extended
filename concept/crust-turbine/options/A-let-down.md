# Option A — the Let-Down

**One line.** §3.2 as written: a long low body with a frosted expansion housing at one end, an inline rotor casing down the middle, and a generator can at the other.

**What moves:** a rotor face behind a grille — roughly **1.4 tiles across on a 5-tile body,
about 25 % of the box**. Small, and honest about it: what this machine actually
does is drop a pressure, and a pressure drop has no moving picture.

## The idea

§3.2 drawn exactly as written, carried as the fair comparison.

A long low body lies along the 2 × 5. At one end a cast **let-down stage** — the
expansion housing where the pressure drops — carries a heavy bloom of frost. Down
the middle runs a smooth pressure-rated **rotor casing** with a small grille in
its flank, and at the far end sits a **generator can**, plainly electrical, with a
brass terminal box.

Identical bolted flanges close both ends.

## Why it suits this machine

**It reads left to right as the process itself**: gas in, pressure dropped,
rotor turned, current out. A player who looks at it once understands what it
does, which is not true of a barrel or a stack.

**The frost is at one end and the electrics at the other**, so the machine has a
direction without needing a label — and that also tells the player which way the
gas is going in a chain.

**And it is the least likely of the five to be mistaken for a steam turbine**,
because the hot end of a steam turbine is where this one is frozen. The
substitution is the whole joke and it lands.

## What it risks

**A long body with a can on one end is a locomotive**, or a compressor set, or
a genset on a skid. It is the most generic shape in the round.

**Twenty-five per cent of the box moves**, well under vanilla's band, and the
moving part is a small disc behind a grille on the flank — the least visible
place on a building the camera looks down at.

**And it is three distinct lumps in a row**, which at 2 × 5 leaves each of them
small. The other options here commit to one idea across the whole length.
...
== BUILDING ==
Crust Turbine -- option A, the Let-Down.
...
== FORM ==
- A LONG LOW BODY lying along the 2x5, riveted, flat -- the flattest building
  in the set
- At the LEFT end a cast LET-DOWN STAGE: a heavy expansion housing, bulbous,
  carrying a thick bloom of pale FROST that runs back along the body a little way
- Down the middle a smooth pressure-rated ROTOR CASING with a small louvred
  GRILLE in its near flank, a dark rotor face visible behind it
- At the RIGHT end a GENERATOR CAN, plainly electrical, ribbed, with a brass
  TERMINAL BOX and cleated cable runs
- IDENTICAL bolted FLANGES closing both ends, square to the tile edges
- A small round white STATUS LENS on the rotor casing, near flank
- Riveted seams, bolt lines, lifting lugs, honest wear

== COLOUR ==
- Body and rotor casing: #4A463F to #6E685C, dark warm iron-nickel
- Generator can: #5A5047, a shade apart from the body so the electrical end reads
- Flanges and fittings: #8A857C, brighter than the body
...
- Two close-up detail panels: the frosted let-down stage where the rime is thickest; and the louvred grille with the rotor face behind it
...
- A FOUR-FRAME MOTION ROW of the same view: rotor a quarter turned; half; three quarters; back to start -- a continuous spin, with the frost unchanged in all four
...
in the style of a game art bible page. Title the sheet CRUST TURBINE -- LET-DOWN -- use

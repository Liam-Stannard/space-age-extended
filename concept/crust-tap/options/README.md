...
| **`pumpjack`** | This building's own style reference, per the reference plan in the art backlog (now `TODO.md`). It is the wellhead read, and it is also a 3 x 3 machine — so take the *finish*, not the scale |
...
The sheets are at `concept/crust-tap/options/`, and
...
`concept/crust-tap/five-options-contact-sheet.png`.

The adopted sheet is `concept/crust-tap/adopted/A-sheet.png`.

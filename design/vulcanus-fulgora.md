...
- **Degradation, not destruction.** A platform that stops catching oxide asteroids limps rather than dies — it can still make port. At the current heating rate (§9.4) Ice alone cannot hold the optimal band under full load — it slows the climb, buying time — and a connected heat-pipe block is the channel that can carry the full thermal load; whether plain pipes reach a stable equilibrium or only delay overheat at these numbers has not yet been measured, but more/better-distributed pipes always help, and it's never nothing.
- **Cooling throughput becomes the scaling decision**, not generator count. Run lean and accept lower steady-state output, or build out the cooling — a heat-pipe block, with Ice on top — to hold peak efficiency.
...
- The generator's `heat_buffer` numbers (`max_temperature` 2000, `specific_heat` 40kJ, `max_transfer` 10MW) are provisional. `specific_heat` against the 4MW fuel draw sets the heating rate at 100°/s (the buffer was verified in-engine, at an earlier value, as exactly linear: ΔT = energy / specific_heat): 6s from cold to the top of the optimal band, 8s to the overheat threshold. That is deliberately faster than Ice alone can hold — Ice removes at most 2 units × 40° = 80°/s, so under full load it only slows the climb (net +20°/s at the cap, ~40s cold to overheat, and the 100-Ice tank lasts 50s at that draw) and can never hold the optimal band by itself. The heat-pipe channel (`max_transfer` 10MW, 2.5× the fuel input) is the only cooling avenue that can, and attached heat pipes add thermal mass of their own (1MJ each, 25× the generator's buffer), so a connected thermal block is *the* way to run at full load — the first playtest's verdict on the previous 400kJ (10°/s, ~80s to overheat, 0.25 Ice/s to hold) was that it heated so slowly Ice trivially held it and heat pipes were pointless. Whether the heat-pipe channel reaches a stable equilibrium or only delays overheat — plain pipes redistribute heat without dissipating it — has not been measured at these numbers; the earlier 12-point measurements (~50–55s to decline, ~100–105s to floor, against the old 3MJ/50°/s figures) no longer apply. The 4x4 footprint's 16 tile-centre connection points *have* been verified in-engine: pipes on every edge tile conduct, pipes on the diagonal corner tiles do not.

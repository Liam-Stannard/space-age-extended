...
| **`chemical-plant`** | This building's own style reference, per the reference plan in `graphics/TODO.md`. Every one of the nine gets a different one, so the set does not come back looking like one machine drawn nine times |

...
| ---------- | --- | ----- |
| `assembling-machine-3` | The house camera, and this building's own assigned style reference in `graphics/TODO.md`'s plan | first |
...
`graphics/TODO.md`'s plan names the v2 sheet for this building; the Vacuum Furnace
...
for opt in A-sealed-hammer; do   # the round's other four pages are deleted; see CHOSEN below
  tools/generate-building-art.py graphics/drop-crusher-options/$opt.md \
      --only sheet --tag ${opt%%-*} --size 1536x1024 --quality high \
      --background opaque \
      --attach /tmp/refs/assembling-machine-3.png \
      --attach /tmp/refs/foundry.png \
      --attach graphics/entity/ignition-array/concept/adopted/R-sheet.png \
      --outdir graphics/entity/drop-crusher/concept/options   # round scratch; the winner moves to concept/adopted/
done
```
...
rejected art; the lesson is recorded here and in `graphics/TODO.md`'s reference
...
`graphics/entity/drop-crusher/concept/adopted/A-sheet.png`, and

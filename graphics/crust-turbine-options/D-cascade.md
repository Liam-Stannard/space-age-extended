# Option D — the Cascade

**One line.** Four expansion stages in a row, each smaller and frostier than the last, so the pressure drop itself is the building and the frost is the animation.

**What moves:** frost advancing stage by stage down the body — roughly **4.2 tiles of a
5-tile body, about 85 % of the box**. The only option in the round inside
vanilla's band, and the only one whose animated element is the process rather
than a part.

## The idea

Make the pressure drop the building.

**Four cast expansion stages** stand in a row along the 2 × 5, joined by short
bolted spools: the first fat, each one after it a little smaller, the last barely
half the first. Gas enters big and hot-pressured and leaves small and spent, and
every stage takes a bite out of it.

**The frost is the animation.** At rest the machine is bare metal. As it runs,
rime creeps from the last stage backward up the cascade — the coldest end first,
because that is where the gas has already expanded — until all four are white.
Nothing rotates.

## Why it suits this machine

**It is the only option whose animation is the physics.** A rotor is a detail
this machine happens to contain; the cascade *is* the pressure let-down, and
watching frost climb it is watching the machine work.

**Eighty-five per cent of the box carries the read**, inside vanilla's band, on a
building whose other options are all well under it. And it costs no new geometry —
frost is a mask over one plate, the cheapest animation in this repo's toolkit.

**Four stepping stages is a silhouette nothing else has**, on this planet or in
vanilla. A steam turbine steps *up* into lagged casings; this steps *down* into
bare cold ones, which is the confusion test answered by the shape itself.

## What it risks

**§9 says frost should track state, not time** — *"frost forming and melting on
a loop reads as the machine stopping and starting"* — and this option animates
frost. That is a direct argument with the brief and it has to be won, not
finessed: the defence is that the frost advances and then *holds* while running,
rather than cycling, and only retreats when the machine stops. If the sheet comes
back with it pulsing, the option is wrong.

**Four stages on five tiles is four small things.** Each stage gets barely a tile,
and at 32 px they may merge into one lumpy body.

**And a stepped row of vessels is a refinery**, or a compressor train, which is
industrial vocabulary this planet has not otherwise used.
...
== BUILDING ==
Crust Turbine -- option D, the Cascade.
...
== FORM ==
- FOUR CAST EXPANSION STAGES in a row along the 2x5, joined by short bolted
  spools. The first is FAT and each one after it is SMALLER -- the last barely
  half the width of the first. They step DOWN from left to right
- The stages are BARE COLD CASTINGS: no lagging, no insulation, no blankets
- FROST creeping from the RIGHT-HAND (last, smallest) stage BACKWARD up the
  cascade, heaviest at the right and thinning to bare metal at the left
- A compact GENERATOR CAN slung under the middle of the row, with a brass
  TERMINAL BOX
- IDENTICAL bolted FLANGES closing both ends, square to the tile edges -- the
  first stage's inlet and the last stage's outlet
- A small round white STATUS LENS on the largest stage
- NO grille, NO rotor face, NO visible rotation anywhere
- Riveted seams, bolt lines, lifting lugs, honest wear

== COLOUR ==
- Stages, spools and can: #4A463F to #6E685C, dark warm iron-nickel, bare
- Flanges and spool bolts: #8A857C, brighter than the body
...
- Two close-up detail panels: the smallest, frostiest stage at the right-hand end; and the step between the first and second stages with their bolted spool
...
- A FOUR-FRAME MOTION ROW of the same view: all four stages bare metal; the last stage rimed; frost reaching the third and second; all four white and HOLDING -- it advances and holds, it does not pulse
...
in the style of a game art bible page. Title the sheet CRUST TURBINE -- CASCADE -- use

# Option B — the Frost Barrel

**One line.** One fat insulated barrel slung between two flanges, frost blooming along its whole length, and nothing visible moving anywhere.

**What moves:** **nothing.** This is the option that takes the template's other honest answer
— decide the machine is still and say so — on a building whose signature is a
state rather than a motion.

## The idea

Stop pretending a pressure drop is a picture.

One fat **barrel** fills almost the whole 2 × 5, slung low between two heavy end
flanges on cast saddles. Its whole length is rimed: frost heaviest at the inlet
end and thinning toward the outlet, so the barrel itself shows the pressure
falling along it.

There is no grille, no window, no rotor face and no visible mechanism. A brass
terminal box on the flank is the only sign it makes electricity at all.

## Why it suits this machine

**Frost along the body is the pressure gradient made visible**, which is a more
honest picture of this machine than a spinning disc: the rotor is a detail, the
drop is the point.

**It is the simplest silhouette in the round and the most distinct from a steam
turbine**, which is all lagged pipework and stepped casings. A bare cold barrel
looks like nothing else on the floor.

**And it is the cheapest to produce and to keep correct** — two plates, two
shadows, a lens, no frames. On a building the brief insists should stay small,
that proportionality is a feature.

## What it risks

**A barrel between two flanges is a pipe fitting, not a machine.** At 2 × 5 and
low, it risks reading as a length of plumbing the player has already built rather
than as a thing to build.

**Nothing moving means the status lens carries the entire running-versus-stopped
read**, on a small building where the lens is a couple of pixels.

**And it throws away the rotor**, which is the one thing this machine has that
would honestly animate — a face pointed at the camera, the motion this project's
tooling derives most cleanly. That is a real cost and it is why this option is
here to be argued with rather than assumed.
...
== BUILDING ==
Crust Turbine -- option B, the Frost Barrel.
...
== FORM ==
- ONE FAT BARREL filling almost the whole 2x5, slung LOW between two heavy end
  flanges on cast saddles. Riveted, plain, no stepped casings
- FROST along its whole length: HEAVIEST at the left end and THINNING toward the
  right, so the barrel shows the pressure falling along it
- NO grille, NO window, NO rotor face, NO visible mechanism of any kind
- A brass TERMINAL BOX on the near flank with cleated cable runs -- the only sign
  it makes electricity
- IDENTICAL bolted FLANGES at both ends, square to the tile edges, on cast saddles
- A small round white STATUS LENS on the terminal box
- Riveted seams, bolt lines, lifting lugs, honest wear. This machine has NO
  moving parts visible anywhere

== COLOUR ==
- Barrel, saddles and flanges: #4A463F to #6E685C, dark warm iron-nickel
- Terminal box: #5A5047 with brass furniture, so the electrical end reads
...
- Two close-up detail panels: the barrel's left end where the rime is thickest, against its bare right end; and the terminal box with its cable cleats
...
in the style of a game art bible page. Title the sheet CRUST TURBINE -- FROST BARREL -- use

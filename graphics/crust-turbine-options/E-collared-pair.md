# Option E — the Collared Pair

**One line.** Built to echo the Crust Tap's Bolted Collar so the two read as one installation that arrived in the same crate.

**What moves:** a rotor face behind a collar-framed window — roughly **1.4 tiles, about
25 % of the box**, the same modest share as option A.

## The idea

The tap and the turbine are always built together and always stand together.
Make them look it.

The adopted tap is the **Bolted Collar**: a heavy machined collar with a ring of
bolts, standing on its vent. This turbine takes that collar and uses it three
times — as the **two end flanges**, and again as a **collar-framed window** in the
middle of the body with the rotor face behind it.

The body between them is plain riveted plate, low and quiet, so the collars carry
the identity. The frosted let-down stage sits under the left collar rather than
beside it, so the machine's outline stays simple.

## Why it suits this machine

**Nothing else in the round tries to be a set**, and these two buildings are a
set: one taps, one burns, and a player never builds one without the other. §20 of
the turbine's brief asks explicitly whether the pairing should be pushed, and this
is the option that answers yes.

**A repeated collar is a cheap, strong motif** that survives at any zoom — three
bright machined rings on a dark body reads from across the base.

**And it solves the flange problem for free.** The two ends must be identical so
the player can see the machines chain; if the flange is already the building's
signature detail, that identity is doing double duty.

## What it risks

**A turbine that borrows its sibling's face may read as one machine cut in
half**, which is the risk §20 names in as many words. Standing side by side they
could look like two halves of something that failed to assemble.

**Three collars on five tiles is a lot of the same thing.** The motif could tip
from family resemblance into monotony, and the body between them is deliberately
plain, which leaves little else to look at.

**And it is the least distinct from a steam turbine of the five** — a plain
riveted body with round fittings is close to vanilla's own shape. The frost has to
carry the whole confusion test on its own here, where the other options get help
...
== BUILDING ==
Crust Turbine -- option E, the Collared Pair.
...
== FORM ==
- A LONG LOW BODY lying along the 2x5, plain riveted plate, deliberately quiet
- THREE HEAVY MACHINED COLLARS, all the same design: a bright machined ring with
  a full circle of bolts round it. Two of them ARE the end flanges; the third
  frames a WINDOW in the middle of the near flank with a dark ROTOR FACE behind
  it
- The LET-DOWN STAGE sits UNDER the left-hand collar rather than beside it,
  keeping the outline simple, with pale FROST blooming from beneath it
- A compact GENERATOR CAN at the right, low, with a brass TERMINAL BOX
- The two end collars are IDENTICAL and square to the tile edges, so it is
  obvious these machines chain end to end
- A small round white STATUS LENS on the middle collar's rim
- Riveted seams, bolt lines, lifting lugs, honest wear

== COLOUR ==
- Body and can: #4A463F to #6E685C, dark warm iron-nickel, plain
- Collars: #8A857C, bright machined rings, deliberately the brightest thing on
  the building, with brass bolt heads at #8A5A32 to #C88A4A
...
- Two close-up detail panels: one collar's machined rim and full circle of bolts, close; and the middle collar framing the rotor face with frost beneath the left one
...
- A FOUR-FRAME MOTION ROW of the same view: rotor a quarter turned behind its collar; half; three quarters; back to start -- a continuous spin, the collars and frost unchanged
...
in the style of a game art bible page. Title the sheet CRUST TURBINE -- COLLARED PAIR -- use

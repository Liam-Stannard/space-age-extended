# Option D — the Cascade Tower

**One line.** A tall narrow tower with a bucket-elevator leg up one flank: dross is lifted once and falls through baffles all the way down.

## The idea

Every other machine on the Core is low. This one goes up: a slab-sided
tower about twice as tall as the footprint is wide, with a bucket-elevator leg
climbing one flank to a head pulley at the top. Everything after the lift is
free -- the material falls through a stack of internal baffle decks, and each
deck's outlet is a closed hopper on the tower's flank.

Gravity does the sorting. The only power in the building is the lift.

## Why it suits this machine

It draws the mod's own argument: on a world at gravity 50, the expensive
part of any process is *raising* something, and everything after that is free.
The machine is a lift plus a fall.

It gives the Core's factory floor a vertical element without an animation budget
-- the elevator's head pulley turns, and that is the whole working read.

## What it risks

**Height is not free at 3x3.** A tall building leans into frame under
Factorio's camera, and the near face grows; drawn badly it becomes a front
elevation with no roof, which is exactly the failure the template's rule zero
warns about. The prompt keeps the roof dominant and states the ratio.

**It could read as a silo or a water tower** -- a tall closed vessel with a leg
up the side is dangerously close to farm architecture. The square-sectioned
slab-sided body, the baffle-deck hoppers stepping down the flank and the open
lattice of the elevator leg are the defence.
...
== BUILDING ==
Dross Classifier -- option D, the Cascade Tower.
A three-by-three classifier built upward: a slab-sided tower with a bucket
elevator up one flank, sorting by letting material fall through internal decks.

== FORM ==
- A square-sectioned tower filling the 3x3 footprint, roughly twice as tall as
  the base is wide, with flat slab sides and a flat roof -- not a cylinder, not
  a cone, not a tapering silo
- A BUCKET ELEVATOR leg climbing one flank in an open bolted lattice casing,
  ending in a guarded head-pulley housing at the roof line
- Four closed hoppers stepping down the opposite flank, one per internal deck,
  each with a bolted access door, all drawn EMPTY and shut
- A low skirt at the base where the tower meets its footprint, with bolt rings
- Roof: a hatch, a hand rail and the head-pulley housing -- the roof is still
  the dominant surface even though the building is tall
- Ladder rungs up one corner, for human scale
...
- FORBIDDEN READS: it must not read as a belt splitter -- not flat, not low, not
  ...
  It must NOT read as a grain silo, a water tower or a chimney: nothing round,
  nothing tapering, no conical roof, no legs holding a vessel off the ground.
...
- Two close-up detail panels: the head-pulley housing at the top of the elevator leg; and two of the stepped flank hoppers and their access doors
- A two-frame state pair: the elevator's head pulley at two points in its rotation
- A palette strip of six colour swatches

== OUTPUT ==
One landscape concept sheet, 3:2, labelled panels on a dark charcoal background,
in the style of a game art bible page. Title the sheet DROSS CLASSIFIER -- CASCADE TOWER -- use exactly

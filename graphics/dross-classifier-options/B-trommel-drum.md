# Option B — the Trommel

**One line.** A long screen drum lying across the footprint in a roller cradle, turned by a ring gear at one end.

## The idea

Stop shaking the machine and turn it instead. A trommel is a cylindrical
screen lying almost horizontal: material tumbles down its length, the fine stuff
falls through the mesh into the shell under it and the coarse stuff walks out the
far end. It is the oldest sorting machine there is, and it is a *cylinder* -- a
silhouette nothing else on the Core has.

The drum is inside a half-shell housing, so what shows is the drum's two end
plates, the ring gear and the roller cradle it rides on.

## Why it suits this machine

It sorts by size and by fall, which is what the recipe says, and the
rotation is legible from outside without opening anything: a ring gear turning
against a pinion is unmistakably a machine at work.

It also solves the family problem. The Ballast Drill, the Drop Crusher and this
building all lean on gravity, and the spec wants them to read as a family -- but
a family of three identical grey boxes is not a family, it is a repetition. A
lying cylinder is the same argument in a different sentence.

## What it risks

**A drum can read as a mixer, a kiln or a cement truck**, all of which are
about combining rather than separating. The two-diameter shell -- fat at the feed
end, narrow past the mesh -- is what says *sorting*, and it has to survive.

**A cylinder at 3x3 is tight.** The drum has to lie inside the footprint with its
cradle, which means it cannot run the full diagonal, and a generator will want to
draw it longer than the box allows.
...
== BUILDING ==
Dross Classifier -- option B, the Trommel.
A low three-by-three classifier built around a rotating screen drum lying on its
side in a roller cradle.

== FORM ==
- A horizontal cylindrical drum lying across the footprint, slightly inclined,
  its axis parallel to one face of the square base -- never diagonal
- The drum is enclosed in a heavy half-shell housing: what shows is the drum's
  two end plates, a short band of it through a guarded opening in the housing
  roof, and the shadow gap where it turns
- A toothed RING GEAR around the drum at the high end, driven by a small pinion
  and a guarded motor housing bolted to the base
- Four roller pairs in a cradle under the drum, visible at the housing's flanks
- The housing tapers: deep and wide at the feed end, shallow at the discharge
  end, so the machine reads as sorting rather than as mixing
- A closed EMPTY discharge bin at the low end and a bolted inspection door
...
- FORBIDDEN READS: it must not read as a belt splitter -- not flat, not low, not
  ...
  It must not read as a concrete mixer, a rotary kiln or a tank: the two-diameter
  housing and the exposed ring gear are what prevent that.
...
- Two close-up detail panels: the ring gear meshing with its pinion; and one roller pair in the cradle under the drum
- A two-frame state pair: the drum at two points in its rotation, the ring gear's teeth advanced between them
- A palette strip of six colour swatches

== OUTPUT ==
One landscape concept sheet, 3:2, labelled panels on a dark charcoal background,
in the style of a game art bible page. Title the sheet DROSS CLASSIFIER -- TROMMEL -- use exactly

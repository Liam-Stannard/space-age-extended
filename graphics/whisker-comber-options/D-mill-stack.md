# Option D — the Mill Stack

**One line.** A rolling-mill stand: two heavy screw-down caps on top, a stack of rollers behind an armoured face, and a stepped guard.

## The idea

Borrow the rolling mill. Two massive screw-down caps stand proud of the
roof at the top of a pair of housings, a stack of rollers sits between them
behind an armoured face, and the whole thing is braced back to a heavy base.
Alignment by drawing the material between rollers under load -- and the *load* is
what the shape is about: screw-downs, thick housings, a machine built to be
squeezed.

## Why it suits this machine

It is the most obviously *strong* machine of the five, and this recipe is
about strength: whiskers are absurdly strong along one axis, and a mill stand is
the shape that says a force is being applied in one direction.

The screw-down caps also give the building something Factorio likes and this mod
lacks -- a piece of machinery standing above the roofline that is unmistakable in
silhouette.

## What it risks

**A mill stand is close to the Drop Crusher's territory**: both are about
squeezing. They must not read as siblings, and the defence is that a crusher is
about impact and this is about a continuous draw -- rollers and screws, not a
falling weight.

**Screw-downs are vertical**, and a tall element on a 3x3 leans into frame and can
eat the roof. The prompt caps the height at about one and a half times the base.
...
== BUILDING ==
Whisker Comber -- option D, the Mill Stack.
A three-by-three fibre-drawing machine built as a rolling-mill stand: paired
housings, screw-down caps, and a roller stack behind an armoured face.

== FORM ==
- A heavy base filling the 3x3 footprint, with two thick vertical HOUSINGS
  rising from it, one on each side, no taller than about one and a half times
  the base's width
- Two SCREW-DOWN CAPS standing proud at the top of the housings: machined
  cylindrical bosses with hexagonal drive heads and thread guards
- Between the housings, an armoured face with a horizontal viewing gap through
  which the ends of a stack of ROLLERS are visible -- polished cylinders in
  bearing chocks, stacked vertically, EMPTY between them
- A guarded drive housing on one flank with a coupling cover
- An infeed guide at one end and an outfeed guide at the other, both EMPTY and
  both guarded
- Tie bars bracing the housings back to the base; flush service panels
...
- FORBIDDEN READS: it must NOT read as a textile mill -- no wooden framing, no
  ...
  It must not read as a press, a stamp or a falling-weight crusher -- there is no
  ram, nothing lifted and nothing dropped. It must not read as a printing press
  either.
...
- Two close-up detail panels: one screw-down cap with its drive head and thread guard; and the roller stack seen end-on through the viewing gap, empty
- A two-frame state pair: the rollers at two points in their rotation, the chocks and screws unmoved
- A palette strip of six colour swatches

== OUTPUT ==
One landscape concept sheet, 3:2, labelled panels on a dark charcoal background,
in the style of a game art bible page. Title the sheet WHISKER COMBER -- MILL STACK -- use exactly
...
`graphics/entity/whisker-comber/concept/options/D-sheet.png`, generated 2026-09-08 from the prompt above with the four attachments named
in the README.

**Measured:** luminance **61.3**, saturation **0.165**, edge density **0.122**.
...
**What landed.** Two screw-down caps standing proud above paired housings, a
roller stack visible end-on through the viewing gap, tie bars back to the base.
The caps give it something the rest of the set lacks: a piece of machinery above
the roofline that is unmistakable in silhouette.

**What to weigh.** It reads as *squeezing*, and so does the Drop Crusher; the two
are close cousins, which was the risk going in. Nothing about it says fibre.

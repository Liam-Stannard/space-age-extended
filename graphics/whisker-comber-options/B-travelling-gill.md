# Option B — the Travelling Gill

**One line.** A long armoured track with a comb head that walks its length, seen through a slot in the roof.

## The idea

A gill box rather than a carding machine: instead of drums, a single comb
head travels the length of the machine on a heavy track, dragging its needles
through the material and lifting clear at the end of each pass. The track is
armoured over, and a narrow slot along the roof lets the head be seen moving --
which means the machine's working state is a *position*, and a player can see how
far through a batch it is.

One moving thing, travelling in a straight line, visible from directly above.

## Why it suits this machine

It uses the camera instead of fighting it. Factorio shows mostly roof, and
this is the only one of the five whose signature feature is on the roof and
therefore always in view.

A travelling head also gives an honest, cheap animation: the head walks, returns,
walks again, and nothing else on the building has to move.

## What it risks

**A slot with something sliding in it can read as a printer, a plotter or
a gantry saw.** The needle bar hanging under the head, and the heavy screw drive
along the track, are what keep it a comb.

**A long slot is a long empty channel**, and nothing may be drawn in it. That is
the sharpest version of the no-product rule on this building: the channel is
...
== BUILDING ==
Whisker Comber -- option B, the Travelling Gill.
A low three-by-three fibre-combing machine built as an armoured track with a
travelling comb head running along it.

== FORM ==
- A machined casing filling the 3x3 footprint, its roof carrying one long
  straight SLOT running end to end, parallel to a face of the square base
- A COMB HEAD sitting in the slot: a heavy carriage with a downward needle bar,
  guarded on both sides, clearly able to travel the slot's length
- A screw drive running the length of the track under a bolted cover, with a
  guarded motor housing at one end
- Two hardened stops at the ends of the slot, with buffer blocks
- An infeed opening at one end and an outfeed slot at the other, both EMPTY,
  both guarded
- Flush panels, lifting eyes at the corners, one cable run entering the drive
  housing
...
- FORBIDDEN READS: it must NOT read as a textile mill -- no wooden framing, no
  ...
  It must not read as a printer, a plotter, a 3D printer, a saw table or a CNC
  gantry: the head is short, heavy and hangs a needle bar, and the machine has
  no frame arching over it.
...
- Two close-up detail panels: the comb head's needle bar and its guards; and one end stop with its buffer block and the screw drive's cover
- A two-frame state pair: the head at each end of its travel, the slot empty behind and ahead of it
- A palette strip of six colour swatches

== OUTPUT ==
One landscape concept sheet, 3:2, labelled panels on a dark charcoal background,
in the style of a game art bible page. Title the sheet WHISKER COMBER -- TRAVELLING GILL -- use exactly
...
`graphics/entity/whisker-comber/concept/options/B-sheet.png`, generated 2026-09-08 from the prompt above with the four attachments named
in the README.

**Measured:** luminance **58.1**, saturation **0.176**, edge density **0.125**.
...
**What landed.** The roof slot runs end to end with a guarded comb head in it,
end stops with buffer blocks, and a travel pair that moves the head from one end
to the other -- a working state that is a *position*, visible from directly
above, which is the argument for this option.

**What to weigh.** At 58.1 it is the darkest sheet of the fifteen, well under the
luminance floor. And the slot is a long empty channel: it reads as a machine
waiting for something rather than one working, which is the risk the enclosure

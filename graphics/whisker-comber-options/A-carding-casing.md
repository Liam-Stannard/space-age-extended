# Option A — the Carding Casing

**One line.** A sealed casing with two drum bearing housings bulging from each flank and a hinged inspection lid on top.

## The idea

The needled drums run inside a heavy casing. What shows outside is the
shape of what is inside it: two cylindrical bearing housings bulging from both
flanks, paired, so the drums are legible without being exposed; a hinged
inspection lid on the roof with a small armoured window; and a guarded drive
housing at one end.

This is the v1 draft's direction, carried into the comparison unchanged so the
other four are judged against something rather than against nothing.

## Why it suits this machine

It is the most conservative answer and the safest: the paired bulges are a
distinctive plan silhouette, and they do the work the spec asks for -- the two
drums legible from outside -- with no open mechanism at all.

It also reads correctly at a glance next to the Dross Classifier, which is the
comparison the spec worries about: bulges and a lid, against springs and a slope.

## What it risks

**It is still fundamentally a box.** The bulges are a modifier on a
rectangle, and at Factorio's zoom a modifier can disappear.

**Two drums inside a casing is invisible mechanism.** Nothing on the outside
moves except the drive end, so the machine's working animation is small and
peripheral -- the opposite of the Rocker Beam's whole-body motion.
...
== BUILDING ==
Whisker Comber -- option A, the Carding Casing.
A low three-by-three metal-fibre carding machine: two needled drums sealed inside
a machined casing.

== FORM ==
- A machined casing filling the 3x3 footprint, low and flat-roofed
- Two cylindrical BEARING HOUSINGS bulging from each flank, paired front and
  back, so the two drums inside are legible from outside -- domed caps, bolt
  rings, grease fittings
- A hinged INSPECTION LID on the roof with a small armoured window, dogged shut
  with four toggle latches
- A guarded drive housing at one end with a belt cover and a cooling grille
- An infeed mouth at one end and an outfeed slot at the other, both drawn EMPTY,
  the infeed a plain guarded opening and the outfeed a narrow slot between two
  polished rollers
- Flush service panels on the faces; a hand rail along one flank
...
- Two close-up detail panels: one bearing housing where it meets the casing flank; and the outfeed slot with its pair of polished rollers, empty
- A two-frame state pair: the inspection lid shut, and open on its hinge showing the needled drum surface beneath
- A palette strip of six colour swatches

== OUTPUT ==
One landscape concept sheet, 3:2, labelled panels on a dark charcoal background,
in the style of a game art bible page. Title the sheet WHISKER COMBER -- CARDING CASING -- use exactly

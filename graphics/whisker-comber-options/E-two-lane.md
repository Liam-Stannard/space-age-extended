# Option E — the Two-Lane Machine

**One line.** One casing, two visibly different paths through it: a long fine-toothed lane and a short heavy one, because the recipe is a choice.

## The idea

The comber has two recipes -- comb slowly into tow, or mat quickly into
felt -- and the choice between them is the reason the building exists. So draw
both. The casing carries two parallel working lanes, obviously unequal: a long
lane with a fine-toothed comb section under a slotted guard, and a short lane
with a heavy matting roller under a plain guard. One drive shaft along the flank
runs both, through different gearing.

A player who has never read the recipe list can see that this machine does two
things and that one of them is slower.

## Why it suits this machine

It is the only design in the set that draws the *decision*, which is the
thing the spec says makes this building worth having. Everything else draws a
process.

It also gives the machine an asymmetry that is not arbitrary -- the long lane and
the short lane are legible from any angle, and asymmetry is what keeps a low grey
box from reading as a crate.

## What it risks

**Two lanes can read as two machines bolted together**, or worse, as a
splitter. The single shared drive shaft, the common casing and the one continuous
roofline are what hold it together as one building.

**It risks implying two outputs.** An assembling machine has one output inventory
and no output position, so neither lane may end in a chute aimed at a tile. Both
lanes end inside the casing.
...
== BUILDING ==
Whisker Comber -- option E, the Two-Lane Machine.
A low three-by-three fibre machine with two visibly unequal working lanes under
one casing and one shared drive.

== FORM ==
- One machined casing filling the 3x3 footprint, with a single continuous
  roofline -- clearly ONE machine, not two bolted together
- Two parallel LANES running the casing's length, side by side, obviously
  unequal: the long lane about twice the length of the short one
- The LONG LANE: a fine-toothed comb section under a slotted guard, its needles
  just visible through the slots, with a tensioner at its far end
- The SHORT LANE: a single heavy matting roller under a plain bolted guard, wider
  and blunter than anything in the long lane
- One DRIVE SHAFT running along the outer flank, feeding both lanes through two
  differently-sized gear housings -- the long lane's is the smaller
- Both lanes end INSIDE the casing under a closed hood: no chute, no port, no
  opening aimed at any tile
- One guarded infeed mouth at the head of the machine, EMPTY, feeding both lanes
...
- FORBIDDEN READS: it must NOT read as a textile mill -- no wooden framing, no
  ...
  It must not read as a belt splitter or as two machines joined: one casing, one
  roofline, one drive.
...
- Two close-up detail panels: the long lane's slotted guard with the comb needles visible through it; and the two gear housings on the shared drive shaft, side by side and different sizes
- A two-frame state pair: the machine stopped and running: the two gear housings' shafts advanced by different amounts between the frames
- A palette strip of six colour swatches

== OUTPUT ==
One landscape concept sheet, 3:2, labelled panels on a dark charcoal background,
in the style of a game art bible page. Title the sheet WHISKER COMBER -- TWO-LANE MACHINE -- use exactly

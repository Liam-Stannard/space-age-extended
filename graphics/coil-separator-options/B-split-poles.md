# Option B — the Split Poles

**One line.** Two massive pole pieces facing each other across a narrow gap, with the field standing in the slot between them.

## The idea

Not a hole -- a gap. Two heavy jacketed pole shoes rise from opposite sides
of the chassis and lean toward each other, almost touching, and the ore falls
through the slot between them. The field lives in that slot: a flat blade of cold
light, edge-on to the viewer, a few centimetres wide and the height of the gap.

Where option A is a ring you look through, this is a vice you look into.

## Why it suits this machine

It is the most physically legible of the five. Everyone who has held two
magnets knows what two facing poles do, and the shape says *pulled apart* --
which is the recipe.

A vertical slot of light is also the strongest silhouette-level read available: at
map zoom the ring in option A closes up to a dot, and a bright line does not.

## What it risks

**Two shoulders with a gap is close to a gantry or a doorway**, and a
doorway is architecture, not machinery. The jacketed windings wrapped round each
pole, and the shoes being visibly *shaped* rather than square, are what keep it
mechanical.

**The gap is a place to draw falling ore**, and no ore may be drawn. The prompt
says the slot is empty and that the light is the only thing in it.
...
== BUILDING ==
Coil Separator -- option B, the Split Poles.
A heavy dark three-by-three magnetic separator built as two facing pole pieces
with a narrow working gap between them.

== FORM ==
- A sealed chassis filling the 3x3 footprint, with a broad flat deck
- Two massive POLE PIECES rising from opposite sides of the deck and leaning in
  toward each other, their faces tapering to shaped shoes that stop a hand's
  width apart -- a narrow vertical slot between them, running front to back
- Each pole is wrapped in a heavy copper winding under a pale cryogenic jacket,
  banded with cooling collars
- The slot itself is EMPTY: nothing falling through it, nothing in it but light
- Flat pale-grey bus bars run up the outside of each pole from the deck
- A closed collection housing under the slot, sealed, with a bolted service door
- The whole assembly is symmetrical about the slot and square to the grid
...
- FORBIDDEN READS: it must NOT read as vanilla's electromagnetic plant -- no white
  ...
  loudspeaker.
  It must not read as a gate, a doorway, an arch or a gantry: the poles are
  short, immensely thick, obviously shaped magnet cores, and they carry
  windings.
...
- Two close-up detail panels: one pole shoe's shaped face and its cryogenic collar; and where a bus bar meets the winding at the pole's base
- A two-frame state pair: the machine unlit and lit: the slot dark, then with a steady flat blade of cold light standing in it and no light anywhere else
- A palette strip of six colour swatches

== OUTPUT ==
One landscape concept sheet, 3:2, labelled panels on a dark charcoal background,
in the style of a game art bible page. Title the sheet COIL SEPARATOR -- SPLIT POLES -- use exactly

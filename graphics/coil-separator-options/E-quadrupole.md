# Option E — the Quadrupole

**One line.** A monolithic block with four coil bosses at its corners and a square shaft straight down through the middle.

## The idea

The most advanced-looking answer: a single chamfered monolith, almost
featureless, with four heavy coil bosses set at the corners of its roof and a
square shaft dropping through the centre between them. Four poles, one throat,
and the field standing in the shaft as a cold column seen from above.

Where the other four options show apparatus, this one shows almost nothing and
lets the light do the work -- tier 3's *"machinery implied rather than shown"*
taken as far as it goes without leaving Factorio.

## Why it suits this machine

Its silhouette is the opposite of every other machine on the Core, which
is worth something on its own: a floor of riveted tier-zero boxes with one
seamless block in it reads as progress without a word of text.

The shaft seen from above also uses the camera rather than fighting it. Factorio
draws mostly roof, and this design's signature feature is on the roof.

## What it risks

**Monolithic is one step from featureless.** Set 2 of the Ignition Array's
concepts failed exactly here -- seamless, cold, and measured at half vanilla's
chroma. The prompt keeps the copper on the bosses and the mechanical density in
the chamfers, service panels and cable runs.

**A square hole in a block is a hopper.** The coil bosses, the cable runs feeding
them and the light in the shaft are what stop it reading as a bin.
...
== BUILDING ==
Coil Separator -- option E, the Quadrupole.
A heavy dark three-by-three magnetic separator drawn as a monolithic block with
four corner coil bosses and a square shaft through its centre.

== FORM ==
- A single chamfered monolithic block filling the 3x3 footprint, its roof the
  dominant surface, its faces sloping slightly inward toward the top
- Four heavy COIL BOSSES, one at each corner of the roof: short cylindrical
  housings with visible copper windings under pale jacketing, angled inward
- A SQUARE SHAFT dropping straight through the centre of the roof between them,
  its mouth drawn already foreshortened by the camera, its walls visible a short
  way down and then dark
- Heavy cable runs from each boss converging on the shaft collar, recessed into
  the roof so they sit flush
- Service panels and recessed vents on the faces, flush-fitted, with no
  fasteners showing; one chamfered access door at ground level
- The block is one object: no separate frame, no legs, no skirt
...
- The field stands INSIDE the square shaft, seen from above as a cold column of
  light down the throat. It does not spill onto the roof and it does not light
  the coil bosses.
...
- FORBIDDEN READS: it must NOT read as vanilla's electromagnetic plant -- no white
  ...
  loudspeaker.
  It must not read as a hopper, a bin, a pit or a lift shaft, and it must not
  read as a monument: the coil bosses and cable runs are what make it a
  machine.
...
- Two close-up detail panels: one corner coil boss with its winding and cable run; and the shaft collar where the four cable runs converge
- A two-frame state pair: the machine unlit and lit: the shaft dark, then with a steady cold column of light down it and the roof unchanged
- A palette strip of six colour swatches

== OUTPUT ==
One landscape concept sheet, 3:2, labelled panels on a dark charcoal background,
in the style of a game art bible page. Title the sheet COIL SEPARATOR -- QUADRUPOLE -- use exactly

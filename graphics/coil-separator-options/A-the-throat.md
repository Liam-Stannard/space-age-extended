...
**One line.** A thick copper toroid stood on edge in the middle of the chassis, its aperture facing the camera and the field burning inside the ring.

## The idea

The machine is built around a hole. A heavy toroidal winding stands
vertically at the centre of the footprint with its opening toward the viewer, the
ore path runs through it, and everything else on the building -- bus bars,
cooling fins, chassis -- is apparatus keeping that ring fed and cold.

This is the v1 draft's direction, kept in the comparison so the other four have
something to beat rather than being judged against nothing.

## Why it suits this machine

It puts the mod's own joke where the player looks first. The coil is the
same object the Ignition Array is built from, and a player who has seen a field
coil segment in their inventory recognises it wound into a machine.

The contained glow does real work: light inside a ring, and nowhere else, is a
read no other building on the Core has.

## What it risks

**The aperture fights the camera.** A circle drawn on a machine seen from
above at forty-five degrees is an ellipse, and a generator that draws a true
circle produces a building that looks like it is leaning back. The prompt asks
for the opening already foreshortened.

**A ring facing the viewer is a turbine intake.** That anti-read is one bad
round away at all times, and the defence is the winding: visible copper turns,
not blades, not a spinner, not a grille.
...
== BUILDING ==
Coil Separator -- option A, the Throat.
A heavy dark three-by-three magnetic separator built around a vertical copper
toroid with an open aperture through it.

== FORM ==
- A sealed chamfered chassis filling the 3x3 footprint, low at the flanks and
  rising to a shoulder at the centre
- At the centre, a thick TOROIDAL COIL standing on edge: visible copper windings
  wrapped round a heavy core, its circular aperture facing the viewer and drawn
  already foreshortened by the camera, wider than it is tall
- Heavy flat pale-grey BUS BARS entering the coil from both flanks, obviously
  overbuilt, sweeping down into the chassis
- Cooling fins in a close-packed bank on the coil's outer face
- A closed hooded chamber under the throat where the split happens, sealed, with
  no opening a player could see into
- Chamfered covers and end caps; no fasteners on the primary faces
...
- Two close-up detail panels: the coil's windings where a bus bar lands on them; and the cooling-fin bank on the coil's outer face
- A two-frame state pair: the machine unlit and lit: the same view with the aperture dark, then with the field steady inside the ring and the chassis unchanged
- A palette strip of six colour swatches

== OUTPUT ==
One landscape concept sheet, 3:2, labelled panels on a dark charcoal background,
in the style of a game art bible page. Title the sheet COIL SEPARATOR -- THE THROAT -- use exactly

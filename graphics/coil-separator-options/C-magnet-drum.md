# Option C — the Magnet Drum

**One line.** A sealed drum turning across the machine's face, with the field showing as a thin bright line where the ore rides it.

## The idea

How magnetic separation is actually done: a drum with the magnet inside
it, material fed across its surface, the magnetic fraction sticking to the shell
and riding round to be scraped off while the rest falls straight past. The drum
sits across the front of the chassis behind a guard hood, and the field is a
single thin arc of cold light along the contact line where the ore rides.

The magnet does not turn; the shell does. That is the detail worth putting on the
sheet.

## Why it suits this machine

It is the only option whose working state is *motion* rather than *glow*,
which makes it the cheapest to animate convincingly and the most legible at
distance -- a turning drum reads as running even when the light is off.

A thin arc of light is also the most disciplined use of the mod's violet: a line
rather than a lamp.

## What it risks

**A drum behind a hood is a recycler**, and vanilla's recycler is a real
building the player will have beside it. The defence is scale and finish: this is
one heavy drum with a scraper and a jacketed stator, not a bank of rollers.

**The contact line is a place to draw material**, and none may be drawn. The
light is the read; the ore is not there.
...
== BUILDING ==
Coil Separator -- option C, the Magnet Drum.
A heavy dark three-by-three magnetic separator built around a sealed rotating
drum with a fixed magnet inside it.

== FORM ==
- A sealed chamfered chassis filling the 3x3 footprint
- A single heavy DRUM lying horizontally across the machine's near half, its
  axis parallel to the near face, recessed under a curved guard hood so only the
  front third of its shell shows
- The drum's end plates are visible at both flanks, each with a bearing housing;
  one carries a guarded drive head
- A fixed SCRAPER blade running the drum's length just clear of its shell, with
  a closed EMPTY discharge housing behind it
- A pale cryogenic collar around each end plate, and a bank of cooling fins on
  the chassis behind the drum
- Flat pale-grey bus bars entering the drum's fixed shaft at both ends
...
- The field appears as a THIN ARC of cold light along the drum's shell where it
  passes the scraper, and nowhere else. Not a haze, not a halo, not a glow round
  the whole drum: one narrow bright line.
...
- FORBIDDEN READS: it must NOT read as vanilla's electromagnetic plant -- no white
  ...
  loudspeaker.
  It must not read as vanilla's recycler: not a bank of rollers behind a grille,
  but one heavy jacketed drum.
...
- Two close-up detail panels: the scraper blade standing off the drum's shell; and one end plate with its bearing housing and cryogenic collar
- A two-frame state pair: the drum at two points in its rotation, the shell advanced between the frames and the fixed scraper unmoved
- A palette strip of six colour swatches

== OUTPUT ==
One landscape concept sheet, 3:2, labelled panels on a dark charcoal background,
in the style of a game art bible page. Title the sheet COIL SEPARATOR -- MAGNET DRUM -- use exactly

...
data:extend({
  ...
  },
  {
    ...
    -- this is what makes vanilla's reactor visibly run. Not vanilla's
    -- reactor-lights-color.png directly: that sprite's lit pixels are
    -- baked-in uranium green, and tinting green orange only multiplies it
    -- dark. lights-mask.png is a neutral-luminance copy of it (each pixel
    -- (L, L, L, 255), L = max(R, G, B); generated by
    -- tools/generate-thermionic-graphics.py) that takes its colour from
    -- the tint -- which, with use_fuel_glow_color below, is Magmatic
    -- Core's fuel_glow_color: magma orange {1, 0.45, 0.1}. Size/scale/
    -- shift/blend are vanilla's.
    working_light_picture = {
      filename = "__space-age-extended__/graphics/entity/thermionic-generator/lights-mask.png",
      blend_mode = "additive",
      draw_as_glow = true,
      width = 320,
      height = 320,
    ...
    -- Tint working_light_picture (and colour the reactor's light) with the
    -- burning fuel's fuel_glow_color (prototypes/item.lua's Magmatic Core
    -- sets it to the same magma orange {1, 0.45, 0.1}). The default is the
    -- fallback for a fuel with no glow colour of its own -- only Magmatic
    -- Core can burn here, so it's belt-and-braces, kept identical.
    use_fuel_glow_color = true,
    default_fuel_glow_color = { r = 1, g = 0.45, b = 0.1 },
    -- Light cast on the surroundings while running. Same magma orange as
    -- the fuel glow; per the use_fuel_glow_color docs the fuel's
    -- fuel_glow_color is used "as light color" while burning, so this
    -- colour is what applies otherwise and is kept identical so nothing
    -- shifts either way. Vanilla's reactor declares no `light`; this
    -- intensity/size is a modest first guess to be judged in-client.
    light = { intensity = 0.6, size = 9, color = { r = 1, g = 0.45, b = 0.1 } },
